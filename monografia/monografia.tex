...
\begin{teorema}
    Para quaisquer $p,q\geq2$ inteiros existe um inteiro mínimo $N=r(p,q)$ tal que qualquer coloração de $K_N$ em duas cores possui uma $p$-clique azul ou uma $q$-clique vermelha.
\end{teorema}
\begin{proof}
...
Assim como um grafo, um \textbf{hipergrafo $k$-uniforme} é um par $(V,E)$ em que $V$ é o conjunto de \textbf{vértices} e $E$ é o conjunto de \textbf{arestas}, só que os elementos de $E$ são $k$-uplas não ordenadas de elementos de $V$.

O teorema de Ramsey pode ser generalizado da seguinte forma:

\begin{teorema}
    Para quaisquer $k,p,q\geq 2$ inteiro existe um inteiro mínimo $N=r^k(p,q)$ tal que qualquer coloração dos subconjuntos de tamaho $k$ de $\{1,...,N\}$ possui $p$ subconjuntos de tamanho $k$ azuis ou $q$ subconjuntos de tamanho $k$ vermelhos.
\end{teorema}
\begin{proof}
