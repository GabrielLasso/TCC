...
Com isso, podemos generalizar o teorema de Erd\H os-Szekeres como foi feito em \cite{pentagon}.
...
\begin{teorema}
    Para todo inteiro $l\geq 2$, todo conjunto finito com pelo menos $ES(\frac{}{})$ pontos no plano possui 
    \begin{itemize}
        \item $l$ pontos colineares, ou
...
\begin{proof}
    ...
    Para $i\in[l-1]\setminus\{1\}$, seja $A_i$ os pontos de $P$ da fronteira de $(P\cap A_{i-1}) \setminus A_{i-1}$ e seja $A_l=(P\cap A_{i-1})\setminus A_{i-1}$.
    ...
    Como $A_{i-1}$ contém $\floor{\frac{|A_{i-1}|}{2l-1}}$ conjuntos distintos de $2l-1$ pontos consecutivos, afirmo que
    $$|A_i|\geq\floor{\frac{|A_{i-1}|}{2l-1}}>\frac{|A_{i-1}|}{2l-1}-1$$
    E disso concluímos
    $$|A_{i-1}|<(|A_i|+1)(2l-1)$$
    
    Suponha que para algum $i\in[l-1]$ vale $A_i=\emptyset$. Então $|A_{i-1}| < (2l-1)$, $|A_{i-2}| < (2l-1)^2 + (2l-1)$, $|A_{i-3}| < (2l-1)^3 (2l-1)^2 + (2l-1)$, ..., e é fácil mostrar por indução que
    ...
    Fixe $z\in A_l$. Para $i$ em $[l-2]$, considere os pontos de $A_i$ em sentido horário. Se $x$ e $y$ são dois pontos consecutivos nessa ordem, dizemos que o segmento orientado $\overrightarrow{xy}$ é um arco vazio se $\Delta (x,y,z)\cap A_{i+1}=\emptyset$, sendo $\Delta(x,y,z)$ o triângulo aberto com vértices $x$, $y$ e $z$.
    ...
    \begin{fato}\label{fato1}
        Se $\overrightarrow{xy}$ é um arco vazio e $\overrightarrow{pq}$ é o arco seguinte a $\overrightarrow{xy}$, então $\overrightarrow{pq}$ também é um arco vazio e $conv(\{x,y,p,q\})$ é um quadrilátero vazio.
    \end{fato}
    \begin{proof}
    ...
    \begin{itemize}
        \item \textbf{alinhado à esqueda} se $p\in\Delta[x,y,z]$
        \item \textbf{alinhado à direita} se $q\in\Delta[x,y,z]$
        \item \textbf{alinhado duplamente} se é alinhado à esquerda e à direita.
    \end{itemize}
    \begin{tikzpicture}
    ...
    Se $\overrightarrow{pq}$ o arco seguinte ao arco vazio $\overrightarrow{xy}$, então $\overrightarrow{pq}$ é alinhado à esquerda ou alinhado à direita.
    \end{fato}
    \begin{proof}

    \end{proof}
    Suponha que $A_1$ não contenha arcos vazios. Então para dois pontos consecutivos $x$ e $y$ em $A_1$ temos pelo menos um ponto de $A_2$ em $\Delta(x,y,z)$. Como para todos pares de pontos concecutivos $x,y$ e $u,v$ em $A_1$, $\Delta(x,y,z)\cap\Delta(u,v,z)=\emptyset$, então $|A_2|\geq|A_1|$, o que contradiz a minimalidade de $A_1$.

    Seja $\overrightarrow{x_1y_1}$ um arco vazio em $A_1$ e, para $i\in[l-2]$, seja $\overrightarrow{x_{i+1}y_{i+1}}$ o arco seguinte a $\overrightarrow{x_iy_i}$.

    Pelo fato \ref{fato1}, $\overrightarrow{x_iy_i}$ é vazio.
\end{proof}

\begin{corolario}
    A conjectura \ref{conj1} é verdadeira para $k=5$.
\end{corolario}
